...
def get_num_masks(mask_path, min_instances):
    ...
    :param min_instances: number of min masks per patched image
    :return: original filenames used to copy the whole image over to our image and mask dir
    """
    orig_filename = ''
...
def get_masks_to_be_removed(mask_paths, min_instances):
    ...
    for mask_path in mask_paths:
        low_instance_mask_basename = get_num_masks(mask_path, min_instances)
        if low_instance_mask_basename:
            masks_to_be_removed.append(low_instance_mask_basename)
